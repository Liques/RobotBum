...
namespace TowerBotCL
{
    static class ServerWriter
    {
        ...
        private static string strJSONPath = Environment.GetFolderPath(
        ...
        public static void UpdatePages(List<AlertFilter> listNewAlerts)
        {
            ...
            if (!exists)
                System.IO.Directory.CreateDirectory(@"c:\inetpub\wwwroot\aeroradartk\data\");
            ...
            foreach (Radar radar in Radar.ListRadars)
            {
                try
                {
                    ...
                    WriteFile(@"c:\inetpub\wwwroot\aeroradartk\data\", radar.Name + ".json", JsonConvert.SerializeObject(listAlertByRadar));
                    var listAlertByRadarFiveMinutes = listAlertByRadar.Where(s => s.TimeCreated > DateTime.Now.AddMinutes(-5)).ToList();
                    WriteFile(@"c:\inetpub\wwwroot\aeroradartk\data\", radar.Name + "fast.json", JsonConvert.SerializeObject(listAlertByRadarFiveMinutes));
                }
                catch (Exception e)
                {
                    throw e;
            ...
            WriteFile(@"c:\inetpub\wwwroot\aeroradartk\data\", "general.json", JsonConvert.SerializeObject(listGeneralOnlyImportantAlert));
        }

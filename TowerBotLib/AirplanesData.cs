...
namespace TowerBotLib
{
    ...
    public static class AirplanesData
    {
        ...
        static async public Task<List<AirplaneBasic>> GetAirplanes(string lat1 = "", string lat2 = "", string lon1 = "", string lon2 = "")
        {
            try
            {

                string location = lat1 + "," + lat2 + "," + lon1 + "," + lon2;
                ...
                List<AirplaneBasic> listAirplane = await DesarializeAirplanes(Radar.MakeSpecialRadar(url));

                return listAirplane;
            }
            catch (Exception e)
            {
                ErrorManager.ThrowError(e, "Load Airplane Data (old)");
                return null;
            }
        }
        ...
        static async private Task<List<AirplaneBasic>> DesarializeAirplanes(Radar radar)
        {
            ...
            try
            {
                ...
                if (radar.IsRaw && listAirplanes == null)
                {
                ...
                //else if (listAirplanes == null)
                //{
                //    listAirplanes = new List<Airplane>();

                //    responseBodyAsText = httpClient.GetStringAsync(radar.EndpointUrl).Result;
                ...
                //            listAirplanes.Add(Airplane.ConvertToAirplane(radar, teste, routes_list[i].Key));
                //        }
                //    }
                //}

...
namespace TowerBotLib
{
    public class MessageMaker
    {
        ...
        private const string TagNormalPreposition = "[NormalPrep]";
        private const string TagUnitPreposition = "[UnitPrep]";
        private const string TagRegistrationName = "[RegistrationName]";
        private const string TagFlightName = "[FlightName]";
        private const string TagLanding = "[Landing]";
        private const string TagTakeoff = "[Takeoff]";
        private const string TagCruise = "[Cruise]";
        private const string TagTaxi = "[Taxi]";
        private const string TagTypeLong = "[TypeLong]";
        private const string TagTypeShort = "[TypeShort]";
        private const string TagSpecialDescription = "[SpecialDesc]";
        private const string TagAircraftType = "[AircraftType]";
        private const string TagAirplaneID = "[AirplaneID]";
        private const string TagAirplaneCountry = "[AirplaneCountry]";
        private const string TagChart = "[Chart]";
        private const string TagRunway = "[Runway]";
        private const string TagOrbit = "[Orbit]";
        private const string TagFrom = "[From]";
        private const string TagTo = "[To]";
        private const string TagOverLocation = "[OverLocation]";
        ...
        string nameMask = String.Empty; // nome de apresentação do avião de acordo com os dados
        ...
        string fullNamePrepositionMask = String.Empty; // Preposição que vai ligar a frase
        string fullNameWithPrepositionStarting = String.Empty; // Com a primeira letra maiuscula
        string fullNameWithPrepositionStartingMask = String.Empty; // Com a primeira letra maiuscula
        string fullNameWithPrepositionMiddle = String.Empty; // Com a primeira letra minuscula
        string fullNameWithPrepositionMiddleMask = String.Empty; // Com a primeira letra minuscula
        string shortNameWithPrepositionStarting = String.Empty; // Com a primeira letra maiuscula
        string shortNameWithPrepositionStartingMask = String.Empty; // Com a primeira letra maiuscula
        string shortNameWithPrepositionMiddle = String.Empty; // Com a primeira letra minuscula
        string shortNameWithPrepositionMiddleMask = String.Empty; // Com a primeira letra minuscula
        ...
        string airplaneTypeShortPhraseMask = String.Empty; // 'Com um B7...
        ...
        private void MakeCurrentMessage(AirplaneBasic airplane, Radar radar, RatificationType ratificationType = RatificationType.NoRatification)
        {
            ...
            string airplaneTypeMask = String.Empty;
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(this.Airplane.SpecialDescription))
                {
                    if (this.Airplane.IsSpecial)
                    {
                        ...
                        nameMask = string.Format("{0}, {1} {2}", TagSpecialDescription, TagNormalPreposition, TagRegistrationName);
                    }
                    else
                    {
                        fullName = airplane.SpecialDescription + " (" + this.Airplane.Registration.Name + ")";
                        nameMask = string.Format("{0}, ({1})", TagSpecialDescription, TagRegistrationName);
                    ...
                    if (!String.IsNullOrEmpty(this.Airplane.FlightName))
                    {
                        ...
                        nameMask += string.Format(", ({0})", TagFlightName);
                ...
                else if (this.Airplane.Registration.IsValid && this.Airplane.AircraftType.Type == AircraftModel.AirplaneHeavy)
                {
                    if (!String.IsNullOrEmpty(this.Airplane.FlightName))
                    {
                        ...
                        nameMask = string.Format("{0}, {1} {2} ({3})", TagAircraftType, TagNormalPreposition, TagFlightName, TagRegistrationName);

                    }
                    else {
                        fullName = airplane.AircraftType.Name + " (" + this.Airplane.Registration.Name + ")";
                        nameMask = string.Format("{0} ({1})", TagAircraftType, TagRegistrationName);
                    ...
                    if (ratificationType == RatificationType.NoRatification)
                    {
                        ...
                        nameMask = string.Format("{0}", TagUnitPreposition);

                    }
                    else {
                        fullNamePreposition = "O";
                        nameMask = string.Format("{0}", TagNormalPreposition);
                ...
                else if (!string.IsNullOrEmpty(this.Airplane.FlightName) && this.Airplane.Registration.IsValid && this.Airplane.AircraftType.Type == AircraftModel.Helicopter)
                {
                    ...
                    (!string.IsNullOrEmpty(this.Airplane.FlightName) && this.Airplane.Registration.IsValid))
                {
                    fullName = this.Airplane.FlightName + " (" + this.Airplane.Registration.Name + ")";
                    shortName = this.Airplane.FlightName;
                    fullNamePreposition = "O";
                    nameMask = string.Format("{0} {1} ({2})", TagNormalPreposition, TagFlightName, TagRegistrationName);
                ...
                else if (!string.IsNullOrEmpty(this.Airplane.Registration.Name))
                {
                    ...
                    shortName = this.Airplane.Registration.Name;
                    fullNamePreposition = "O";
                    nameMask = string.Format("{0} {1}", TagNormalPreposition, TagRegistrationName);
                ...
                else if (this.Airplane.AircraftType.IsValid)
                {
                    ...
                    nameMask = string.Format("{0} {1}", TagUnitPreposition, TagAircraftType);
                ...
                else
                {
                    ...
                    fullNamePreposition = "Um";
                    nameMask = string.Format("{0} {1}", TagUnitPreposition, TagAirplaneID);
                ...
                if (ratificationType == RatificationType.NoRatification)
                {
                    if (!this.Airplane.IsKnowCountry)
                    {
                        ...
                        this.Mask += string.Format(" - {0} - ", TagAirplaneCountry);
                    ...
                    if (this.Airplane.FollowingChart != null)
                    {
                        ...
                        this.Mask += string.Format(", {0}", TagChart);
                    ...
                    if (!String.IsNullOrEmpty(this.Airplane.RunwayName) && (this.Airplane.State == AirplaneStatus.Landing || this.Airplane.State == AirplaneStatus.TakingOff))
                    {
                        ...
                        this.Mask += string.Format(", {0}", TagRunway);
                    ...
                    if (this.Airplane != null)
                    {
                        ...
                        if (airplane.State == AirplaneStatus.ParkingOrTaxing)
                        {
                            ...
                            if (!String.IsNullOrEmpty(overLocation))
                            {
                                ...
                                this.Mask += string.Format("{0} {1}", TagOverLocation, overLocation);
                            }
                        }
                        else if (Message.Length <= 110)
                        {
                            Message += TowerBotLib.Filters.HelperFilter.GetForwardLocationsPhrase(this.Airplane, true, 2);

                        }
                        else
                        {
                            Message += TowerBotLib.Filters.HelperFilter.GetForwardLocationsPhrase(this.Airplane, true);
                        }
                ...
                else if (ratificationType == RatificationType.Orbit)
                {
                    ...
                    this.Mask += string.Format("{0}", TagOrbit);
        ...
        private string GetMessageFromMask()
        {
            return string.Empty;
        }
        ...
        private string GetChartPhrase()
        {
            ...
            {
                phrase += TowerBotLib.Filters.HelperFilter.GetForwardLocationsPhrase(this.Airplane, true, 2);
            }
            ...
            switch (place)
            {
                case Place.Miami:
                    if (airplane.FlightName.EndsWith("8042") ||
                        ...
                        airplane.FlightName.EndsWith("AAL213", StringComparison.InvariantCultureIgnoreCase) ||
                        airplane.FlightName.EndsWith("AAL214", StringComparison.InvariantCultureIgnoreCase))
                        return true;
                    break;
                case Place.Orlando:
                    if (airplane.FlightName.EndsWith("8048") ||
                        airplane.FlightName.EndsWith("8049") ||
                        airplane.FlightName.EndsWith("L587", StringComparison.InvariantCultureIgnoreCase) ||
                        airplane.FlightName.EndsWith("L586", StringComparison.InvariantCultureIgnoreCase))
                        return true;
                    break;
                case Place.France:
                    if (airplane.FlightName.EndsWith("R515", StringComparison.InvariantCultureIgnoreCase) ||
                        airplane.FlightName.EndsWith("R520", StringComparison.InvariantCultureIgnoreCase))
                        return true;
                    break;
                case Place.Portugal:
                    if (airplane.FlightName.StartsWith("TAP", StringComparison.InvariantCultureIgnoreCase))
                        return true;
                    break;
